...
4. Launch the GUI from the desktop shortcut (it must run elevated). Set the ERP,
   `apiListen`, DB host/port/user/database and the password. Leave
   **Authentication method** on *Bearer token* and click **Generate key**, then
   **שמירה** (Save).
...
6. If the calling backend authenticates with a username and password rather than
   a static token — erp-manager does — set **Authentication method** to
   *Username and password* instead, set a username, press **Generate** for the
   password, and save. That clears the bearer token: an installation uses one
   credential. The signing secret is generated by the service on its next start.
   See [authentication.md](authentication.md).
7. Click **Start server**, then verify (below).
8. Give the bearer token, and the exchange credentials if you set any, to
   whoever configures the backend. Those must reach the backend **before** it
   starts calling this connector, or its first call is a 401.
...
round-trip), generates a bearer token if none exists, can enable the credential
exchange with `-auth-user` (generating the password and signing secret and
printing them), and can import a
...
# 2. is the service up and is the credential good? (no database touch)
curl.exe -s http://127.0.0.1:8080/api/ping -H "Authorization: Bearer <token>"
...
Neither ever contains the bearer token, the DB password or any exchange
credential. `server.log` is written
