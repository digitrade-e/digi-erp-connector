...
6. If the calling backend authenticates with a username and password rather than
   a static token — erp-manager does — tick **Enabled** under *Credential
   exchange*, set a username, press **Generate** for the password, and save
   again. The signing secret is generated by the service on its next start. See
